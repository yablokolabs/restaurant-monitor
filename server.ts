...
function getExpectedFromHoursText(hoursText: string, detailedHours?: { [key: string]: string }): boolean {
...
async function fetchRestaurantInfo(url: string) {
  ...
  try {
    ...
    const restaurantInfo = await page.evaluate(() => {
      ...
      if (allText.includes('currently open') || allText.includes('open now') || 
          (allText.includes('open') && !allText.includes('closed'))) {
        currently = 'Open';
        isOpen = true;
      } else if (allText.includes('currently closed') || allText.includes('closed now') || 
      ...
      if (timingButton) {
        ...
        const detailedHours = await page.evaluate(() => {
          ...
          if (Object.keys(hoursData).length === 0) {
            const popupElements = document.querySelectorAll('[class*="popup"], [class*="modal"], [role="dialog"]');
    ...
    const dbResults = scrapedData.map(result => {
      ...
      return {
        ...
        opening_hours: JSON.stringify({
          ...
          openingHours: displayHours, // Use detailed hours for display if available
    ...
    const { error } = await supabase
      ...
      .upsert(
        dbResults.map(r => ({
          ...
          actual: r.actual,
          mismatch: r.mismatch,
          url: r.url, // Include the URL in the upsert
        })),
        {
